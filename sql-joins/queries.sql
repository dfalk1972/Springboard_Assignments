...
oins_exercise=# SELECT * FROM owners FULL JOIN vehicles ON owners.id = vehicles.owner_id;
...
joins_exercise=# SELECT first_name, last_name, COUNT(*) AS count FROM vehicles LEFT JOIN owners ON vehicles.owner_id = owners.id GROUP BY first_name, last_name ORDER BY first_name;
...
joins_exercise=# SELECT first_name, last_name, AVG(price)::NUMERIC(10,0) AS average_price, COUNT(*) AS count FROM vehicles LEFT JOIN owners ON vehicles.owner_id = owners.id GROUP BY owners.first_name, owners.last_name  HAVING COUNT(*) > 1 AND AVG(price) > 10000  ORDER BY first_name DESC;
