document.addEventListener("DOMContentLoaded", function () {
  ...
  savedNotes.forEach((noteItem) => {
    const note = document.createElement("textarea");
    note.setAttribute("data-note-id", noteItem.id);
    note.value = noteItem.content;
    note.className = "note";
  ...
  document.addEventListener("dblclick", function (event) {
    if (event.target.classList.contains("note")) {
      ...
      const noteToDelete = event.target.getAttribute("data-note-id");
  ...
  noteContainer.addEventListener(
    ...
    function (event) {
      if (event.target.classList.contains("note")) {
        ...
        const noteToUpdate = event.target.getAttribute("data-note-id");
        const newMessage = event.target.value;
        savedNotes = savedNotes.map(function (note) {
          if (note.id == noteToUpdate) {
            return { id: note.id, content: newMessage };
          }
          return note;
        });
        localStorage.setItem("notes", JSON.stringify(savedNotes));
