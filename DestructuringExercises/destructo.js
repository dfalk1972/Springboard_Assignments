...
console.log(`The x coordinate is ${x} and the y coordinate is ${y}.`);
...
console.log(`The key locations for the quest are ${first} & ${second}. `);
...
console.log(`The riddle is ${translation}, ${modernWord}`);
...
console.log(`The essential elements are ${firstElement} & ${secondElement}`);
...
console.log(
  `The first stone is ${firstStone} and the sixth stone is ${sixthStone}`
);
...
console.log(
  `The visible shadow is ${firstShadow} and the rest of the shadows are ${hiddenShadows}`
);
...
function revealPath({ direction, distance }) {
  console.log(
    `To find your destination you must travel ${direction} for ${distance} miles.`
  );
}

revealPath({ direction: "north", distance: 45 });
...
function mixPotion({
  Ingredient1: Ingredient1 = "Water",
  Ingredient2: Ingredient2 = "Fireflower",
}) {
  console.log(
    `The potion recipe must have the following two ingredients ${Ingredient1} & ${Ingredient2}`
  );
}

mixPotion({ Ingredient1: "Bread", Ingredient2: "Peanut butter" });
mixPotion({});
...
function castSpell([Ingredient1, ingredient2, ingredient3]) {
  console.log(
    `Mix these ${Ingredient1} & ${ingredient2} to cast a spell on your enemies. `
  );
}
castSpell(["Peanut Butter", "Jelly", "Toast", "Eggs"]);
...
console.log(
  `You have unlocked the final door, here is what you need to finish the job "${finalKey}"!`
);
...
let bothStones = [stoneA, stoneB];
[stoneB, stoneA] = bothStones;
console.log(
  `stoneA was set to "Emerald" and stoneB was set to "Ruby", after the swap, stoneA is ${stoneA} and stoneB is ${stoneB}`
);
