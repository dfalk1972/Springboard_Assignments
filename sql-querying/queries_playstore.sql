...
--14. Find all the categories that have more than 300 apps in them.--
    SELECT category,COUNT(app_name) AS num_apps FROM analytics GROUP BY category HAVING COUNT(app_name) > 300;
...
--15. Find the app that has the highest proportion of min_installs to reviews, among apps that have been installed at least 100,000 times. Display the name of the app along with the number of reviews, the min_installs, and the proportion  --
    SELECT app_name, reviews, min_installs, min_installs/reviews AS proportion FROM analytics WHERE min_installs >= 100000 AND reviews >0 ORDER BY proportion DESC LIMIT 1;
...
--FS1. Find the name and rating of the top rated apps in each category, among apps that have been installed at least 50,000 times.
    SELECT a1.app_name, a1.rating FROM analytics a1 WHERE  a1.rating = (SELECT MAX (a2.rating) FROM analytics a2 WHERE a2.category = a1.category AND min_installs >=50000) AND min_installs >= 50000; 

--FS2. Find all the apps that have a name similar to “facebook”.
    SELECT app_name, rating FROM analytics WHERE app_name ILIKE '%facebook%';

--FS3. Find all the apps that have a name similar to “facebook”.
...
--FS4. Find all the apps that have education as one of their genres
    SELECT app_name FROM analytics WHERE 'education' ILIKE ANY(genres);